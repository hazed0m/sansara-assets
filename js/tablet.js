...
// 	$('.container').fadeIn();
// }
$('.news-wrapper .newswrap .section').mousewheel(function(e, delta) {
...
$('.main-wrapper .news, .main-wrapper .goverment, .main-wrapper .cars, .main-wrapper .settings, .main-wrapper .adssell, .main-wrapper .adsbuy, .main-wrapper .adsgetworkers, .main-wrapper .adssearchwork, .main-wrapper .customs, .main-wrapper .business, .main-wrapper .transportCompany, .main-wrapper .newsAgency').on('click',function(){
	...
	{
		if($('.newswrap .section .news-item').length == 0)
		{
			getNews(currentPage);
		}
	}
...
function initNewsAgency()
{
	$('.newsAgency-wrapper .input-wrapper .input-button').on('click',function(){
		...
		if(!$(this).hasClass('downloaded') && !$(this).hasClass('disabled'))
		{
			if(id == 'imageLink')
			{			
				...
				setTimeout(function(){
					$('.newsAgency-wrapper .image-wrapper').removeClass('active');
					$('.newsAgency-wrapper .publishing-wrapper').addClass('active');
				},1300);
			}
			if(id == 'videoLink')
			{
				...
				// 					<div class="input-button disabled"><i class="fas fa-cloud-download-alt"></i></div>
				// 				</div>
				// 			</div>
				// 		</div>`;
				...
				// },1300);				
				setTimeout(function(){
					$('.newsAgency-wrapper .video-wrapper').removeClass('active');
					$('.newsAgency-wrapper .publishing-wrapper').addClass('active');
				},1200);
			}
			setTimeout(() => {
		...
		if(id == 'imageLink')
		{
			...
			{			
				...
				if(link.includes('youtube.com/watch?v='))
				{
					...
					newsObj.imageLink = `http://i3.ytimg.com/vi/${link}/hqdefault.jpg`;
					$('.image-wrapper .image-preview, #image .resource-preview').css({'background':`url(http://i3.ytimg.com/vi/${link}/hqdefault.jpg) center`,'background-size':'cover'});		
				}
				...
				{
					$('.image-wrapper .image-preview, #image .resource-preview').css({'background':`url(${link})`,'background-size':'cover'});
					newsObj.imageLink = link;
				}
			}
			else
			{
				...
				$('#image .resource-preview').css({'background':`#262626 url(../img/tablet/news/picture.png) no-repeat center`,'background-size':'25%'});
				newsObj.imageLink = '';
				$('#image .resource-preview').empty();
			}
		}
		if(id == 'videoLink')
		{
			...
			{	
				...
				{							
					...
					fillOnTheVID(link);
					$('.video-wrapper .download .video-preview, #video .resource-preview').css({'background':`url(http://i3.ytimg.com/vi/${link}/hqdefault.jpg) center`,'background-size':'cover'});		
					let fancyLink = `
						<a data-fancybox href="https://www.youtube.com/embed/${link}" data-id="${link}" class="video-block">
							<i class="fas fa-play"></i>
						</a>`;
					$('#video .resource-preview').empty().append(fancyLink);
					newsObj.videoLink = link;
				}
				else if(link.includes('youtu.be'))
				{							
					$(this).next().removeClass('disabled');
			...
			else
			{
				...
				$('#video .resource-preview').css({'background':`#ff3b3b url(../img/tablet/news/video.png) no-repeat center`,'background-size':'20%'});
				$('#video .resource-preview').empty();
			}
		}
		checkNews(newsObj);
	});
	...
	$('.newsAgency-wrapper #editPreview').on('click',function(){
		let id = $(this).parent().parent()[0].id;
		if(id == 'image')
		{
			$('.container .newsAgency-wrapper .image-wrapper .image-preview').css({'background':`#262626 url(../img/tablet/news/picture.png) no-repeat center`,'background-size':'none'});
			$('.container .newsAgency-wrapper .image-wrapper .input-wrapper input').removeClass('downloaded').val('');
			$('.container .newsAgency-wrapper .image-wrapper .input-wrapper .input-button').removeClass('downloaded').addClass('disabled').html('<i class="fas fa-cloud-download-alt"></i>');
		}
		if(id == 'video')
		{
			console.log($('.container .newsAgency-wrapper .video-wrapper .video-preview'));
			$('.container .newsAgency-wrapper .video-wrapper .title').text('Без названия');
			$('.container .newsAgency-wrapper .video-wrapper .pre-title span').text('Не определено');
			$('.container .newsAgency-wrapper .video-wrapper .video-preview').css({'background':`#ff3b3b url(../img/tablet/news/video.png) no-repeat center`,'background-size':'none'});
			$('.container .newsAgency-wrapper .video-wrapper .input-wrapper input').removeClass('downloaded').val('');
			$('.container .newsAgency-wrapper .video-wrapper .input-wrapper .input-button').removeClass('downloaded').addClass('disabled').html('<i class="fas fa-cloud-download-alt"></i>');
		}
		if(id == 'text')
		{
			$('.newsAgency-wrapper .text-wrapper .editor-wrapper').fadeIn();
		}
		$('.newsAgency-wrapper .menu-block .button.active').removeClass('active');
		$(`.newsAgency-wrapper .workscreen .workscreen-item.active`).removeClass('active');
		$(`.newsAgency-wrapper .workscreen .${id}-wrapper`).addClass('active');
	});
	$('.newsAgency-wrapper .text-wrapper .button-wrapper .button').on('click',function(){
		let id = this.id;
		if(id == 'submit')
		{
			let template = $('#editor .ql-editor').html();
			$('.newsAgency-wrapper .complete-text, #text .resource-preview').html(template);
			newsObj.text = template;
			$('.newsAgency-wrapper .text-wrapper .editor-wrapper').fadeOut();
			setTimeout(function(){
				$('.newsAgency-wrapper .publishing-wrapper').addClass('active');
			// 	$('.newsAgency-wrapper .text-wrapper .change-wrapper').fadeIn().css('display','flex');
			},400);
		}
		if(id == 'cancel')
		{
			let template = $('.newsAgency-wrapper .complete-text').html();			
			$('.newsAgency-wrapper .text-wrapper .editor-wrapper').fadeOut();
			setTimeout(function(){
				$('.newsAgency-wrapper .publishing-wrapper').addClass('active');
			// 	$('.newsAgency-wrapper #editor .ql-editor').html(template);
			// 	$('.newsAgency-wrapper .text-wrapper .change-wrapper').fadeIn().css('display','flex');
			},400);
		}
		if(id == 'change')
		{
			$('.newsAgency-wrapper .text-wrapper .change-wrapper').fadeOut();
			setTimeout(function(){
				$('.newsAgency-wrapper .text-wrapper .editor-wrapper').fadeIn().css('display','flex');
			},400);
		}
		checkNews(newsObj);
	});
	$('.newsAgency-wrapper .employee-wrapper .button-wrapper .button#submit').on('click',function(){
		let name = $(this).parent().parent().find('input').val();
		let template = `
			<div class="employee-item">
				<div class="employee-name">${name}</div>
				<div class="button-wrapper">
					<div class="button" id="sackEmployee">Уволить</div>
				</div>
			</div>`;
		if(name.length > 0)
		{
...
window.addEventListener('message', function(event) {       
	if (event.data['getNews']) {	
		let currentNews = JSON.parse(event.data['getNews']);
...
$('.news-wrapper .section').scroll(function () {
	var $elem = $('.news-wrapper .section');
...
function initNews(currentNews)
{
	$(currentNews).each(function(index,item){
		let template = `
			...
			<a id="more" href="#">Прочитать полностью</a>
			</div>`;
		$('.news-wrapper .section').append(template);
	});	
	$('.newswrap .section .news-text a,	.news-fullscreen .news-text a').on('click', function(e){
		e.preventDefault();
	});
	$('.news-item #more').click(function(e) {	
		console.log('clicknews');
		let currentElem = $(this).parent(),
			text = currentElem.find('.news-text').text(),
			title = currentElem.find('.news-title').text(),
			image = currentElem.attr('data-image'),
		...
		{			
			...
			$('.news-fullscreen .video-block').empty();
			$('.news-fullscreen .video-block').append(`<img style="width:100%;" src="http://i3.ytimg.com/vi/${video}/hqdefault.jpg"><i class="fas fa-play"></i>`);	
		}
		else
		{
			$('a.video-block').css('display','none');
		}
		$('.news-fullscreen .news-title').text(title);
		$('.news-fullscreen .news-text').text(text);
		if(image != '')
		{
			$('a.image-block').css('display','block');
			$('.news-fullscreen .image-block').empty();
			$('.news-fullscreen .image-block').attr('href',image).append(`<img style="width:100%;" src="${image}"><i class="fas fa-search-plus"></i>`);
		}
		else
		{
			$('a.image-block').css('display','none');
			$('.news-fullscreen .video-block').css('margin','0');
		}
		$('.back-but').css('z-index','-1');
		$('.news-fullscreen').fadeIn().css('display','flex');
	});
	$('.button#сlose').on('click',function(){
		$('.news-fullscreen').fadeOut();
		$('.back-but').css('z-index','1');
	});
}
$('.newsAgency-wrapper #publishNews').on('click',function(){
	if(!$(this).hasClass('disabled'))
	{
		...
		window.addEventListener('message', function(event) {       
			if (event.data['published']) {
				$('.newsAgency-wrapper #publishNews').addClass('disabled');		
				console.log('published');
				$('.container .newsAgency-wrapper .workscreen .fullscreen-sucсess').fadeIn().css('display','flex');
				setTimeout(function(){
					$('#title input').val('');
					$('#video .resource-preview').css({'background':`#ff3b3b url(../img/tablet/news/video.png) no-repeat center`,'background-size':'20%'});
					$('#image .resource-preview').css({'background':`#262626 url(../img/tablet/news/picture.png) no-repeat center`,'background-size':'25%'});
					$('#image .resource-preview, .newsAgency-wrapper #editor .ql-editor, #text .resource-preview, #video .resource-preview').empty();
					newsObj.imageLink = '';
					newsObj.videoLink = '';
					newsObj.text = '';	
					$('.container .newsAgency-wrapper .workscreen .fullscreen-sucсess').fadeOut();
				},1500);									
			} 
		});		
	}
...
{
	if((obj.imageLink.length > 0 || obj.videoLink.length > 0) && obj.title.length > 0 && obj.text.length > 0  && obj.text != "<p><br></p>")
	{
		$('.button#publishNews').removeClass('disabled');				
	}
	else
	{
